    ...
    if config_params['Modeling parameters']['Fit'] == True:
        ...
        try:
            for i, rep_parameters in enumerate(minimizer_params):
                for j, parameter in enumerate(rep_parameters):
                    v = rep_parameters[parameter].value
                    u = all_param_units[j]
                    if parameter in opt_params.keys():
                        opt_params[parameter].append(v)
                        print(f"{parameter} = {v} {u}")
                    else:
                        print(f"{parameter} = {v} {u} (fixed)")
